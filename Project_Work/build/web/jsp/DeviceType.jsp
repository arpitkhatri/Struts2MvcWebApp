...
//    },
                    colNames: ['ID', 'Device Type Code', 'Device Type Desc', 'PlatFormID', 'PlatForm Code', 'Active Status', 'Effective Start Date', 'Effective End Date', 'Create Date', 'Updated Date'],
                    colModel: [
                        {name: 'deviceTypeId', index: 'deviceTypeId', editable: true, hidden: true},
                        {name: 'deviceTypeCode', index: 'deviceTypeCode', editable: true,editrules:{required:true},editoptions:{readonly:true},
                            formoptions: {
                                elmprefix: "<span class='mystar' style='color:red'>*</span>&nbsp;"
                            }},
                        {name: 'deviceTypeDesc', index: 'deviceTypeDesc', editable: true, editrules: {required: true},
                            formoptions: {
                                elmprefix: "<span class='mystar' style='color:red'>*</span>&nbsp;"
                            }},
                        {name: 'platformID', index: 'platformID', editable: true, hidden: true},
                        {name: 'platformCode', index: 'platformCode', width: 150, editable: true, editoptions: {size: "20", maxlength: "45"}, editrules: {required: true},
                            formoptions: {
                                elmprefix: "<span class='mystar' style='color:red'>*</span>&nbsp;"
                            }, edittype: "select",
                            editoptions:{
                                dataUrl: 'getPlatFormCode'
                            }
                ...
                jQuery(grid_selector).jqGrid('navGrid', pager_selector,
                        roleBasedNavOption,
                        {
                            ...
                            onclickSubmit: function(params) {
                                params.url = 'updateDeviceType?deviceTypeCode=' + $('#deviceTypeCode').val()+'&deviceTypeDesc'+ $('#deviceTypeDesc').val()+'&platformCode'+ $('#platformCode').val()+'&activeStatus'+ $('#activeStatus').val()+'&effStartDate'+ $('#effStartDate').val()+'&effEndDate'+$('#effEndDate').val()+'&createDate'+$('#createDate').val()+'&updatedDate'+$('#updatedDate').val();
                                params.mtype = 'POST';
                            },
                            recreateForm: true,
                            ...
                            afterSubmit: function(data, postdata) {
                                ...
                                if (result.successEdit != "") {
                                    ...
                                    return[false, "User Details Already Exists !!"];
                                }
                            }
                        }, //end of edit
                {
                    ...
                    reloadAfterSubmit: true,
                    onclickSubmit: function(params) {
                        params.url = 'addDeviceType?deviceTypeCode=' + $('#deviceTypeCode').val() + '&deviceTypeDesc' + $('#deviceTypeDesc').val() + '&platformID' + $('#platformCode').val() + '&activeStatus' + $('#activeStatus').val() + '&effStartDate' + $('#effStartDate').val() + '&effEndDate' + $('#effEndDate').val() + '&createDate' + $('#createDate').val() + '&updatedDate' + $('#updatedDate').val();
                        params.mtype = 'POST';
                    },
                    recreateForm: true,
                    viewPagerButtons: false,
                    beforeShowForm: function(e) {
                          $('#deviceTypeCode').removeAttr('readonly');
                        var form = $(e[0]);
                        form.closest('.ui-jqdialog').find('.ui-jqdialog-titlebar').wrapInner('<div class="widget-header" />')
                        style_edit_form(form);
                    },
                    afterSubmit: function(data, postdata) {
                        ...
                        if (result.successAdd != "") {
                            ...
                            return[false, "User Detail Already Exists !!"];
                        }
                    }
                },
                {
                    ...
                    onclickSubmit: function(params) {
                        ...
                        var cellValue = $(grid_selector).jqGrid('getCell', selectedRowId, 'deviceTypeId');
                        params.url = 'deleteDeviceType';
                        params.mtype = 'POST';
                        return {deviceTypeId: cellValue};
                    },
                    afterSubmit: function(data, postdata) {
                        var result = jQuery.parseJSON(data.responseText);
